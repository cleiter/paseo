...
Status **dots** are the one exception, and they are a family of their own — `statusDotSuccess`, `statusDotDanger`, `statusDotWarning`, `statusDotRunning`, read only by `getStatusDotColor` (`packages/app/src/utils/status-dot-color.ts`). Same hues and the same generation rule, but their own band: pushed away from the surface rather than toward it (darker in light, lighter in dark) and at 90% of gamut chroma against the status family's 55–60%. A dot is a few points of solid color with no shape to read and no label attached, so at the status band's lightness the dots read dimmer than the metadata beside them — backwards, since the dot is the row's state. All four move together; regenerate the set, never one hue.

The running dot is the only animated one, and the animation is strictly outside the dot: an arc orbits it against a faint track (`OrbitStatusRing`, `packages/app/src/components/orbit-status-ring.tsx`). The dot's own fill, opacity, and size never change, and three earlier attempts are why. Fading it blends it toward the row surface rather than toward black, so the low point read as the success dot one row up. Dropping chroma greys it; raising chroma has nowhere to go, since blue at this band's lightness is already within 11% of the sRGB gamut edge. And changing its size reads as jitter down a column of static dots. Those three all failed the same way: they made one channel carry both which state a row is in and whether it is active, so the activity signal ate into the state signal. The ring is a second channel. The dot holds the state at constant fill, the ring carries the activity, and motion cannot be confused with a hue.

A ring was tried once before and dropped for being too subtle, so if you are about to change this one, change it away from what failed rather than back toward it. That version expanded and faded out — a periodic event, empty for most of its cycle, so you had to be looking when it fired. This one turns continuously and never leaves the row unmarked. The faint track is not decoration either: a bare arc on empty background has nothing to be displaced against and disappears at this size. The other half of that failure was the project badge clipping the ring to its 12pt shell; the shell no longer clips, and it still reads as a knockout because its fill is what does that.

Every ring in the app is driven from one refcounted clock, per `docs/coding-standards.md`.
