...
function findPendingPlanRequestId(
  ...
  for (const message of messages) {
    if (
      ...
      message.payload.event.request.kind === "plan"
    ) {
      return message.payload.event.request.id;
    }
  }
  return null;
}
...
describe("daemon E2E (real claude) - send message during tool call", () => {
  ...
  test("a message sent while a plan is pending is answered in the same turn", async () => {
    ...
    try {
      ...
      if (!client) throw new Error("Claude plan steering test client was not created");
      await within("connect Claude plan steering client", 15_000, client.connect());
      await within(
        "subscribe Claude plan steering client",
        15_000,
        client.fetchAgents({ subscribe: { subscriptionId: "plan-steer" } }),
      );
      const agent = await within(
        "create Claude plan-mode agent",
        30_000,
        client.createAgent({
          cwd: cwd ?? process.cwd(),
          title: "claude-plan-steer",
          ...getRealProviderConfig("claude"),
          modeId: "plan",
        }),
      );
      resources.collector = createMessageCollector(client);
      const collector = resources.collector;

      await within(
        "submit Claude plan turn",
        30_000,
        client.sendAgentMessage(
          agent.id,
          // Leave nothing to ask about: with a vaguer prompt the model asks a
          // clarifying question instead, and the turn parks on that rather
          // than on a plan.
          "Do not ask me any questions. Plan this single change: append the line " +
            "'Hello' to README.md in this directory. Present that plan for approval now.",
          { messageId: generateClientMessageId() },
        ),
      );

      const planRequestId = await within(
        "wait for the Claude plan approval card",
        180_000,
        (async () => {
          const deadline = Date.now() + 170_000;
          while (Date.now() < deadline) {
            const pendingPlanId = findPendingPlanRequestId(collector.messages, agent.id);
            if (pendingPlanId) {
              return pendingPlanId;
            }
            await sleep(100);
          }
          throw new Error("Claude never presented a plan for approval");
        })(),
      );

      const messagesBeforeSteer = collector.messages.length;
      await within(
        "submit a message instead of approving the plan",
        30_000,
        client.sendAgentMessage(agent.id, "Before implementing, reply exactly: PLAN_REVIEWED.", {
          messageId: generateClientMessageId(),
      ...
      const finish = await within(
        "wait for the steered plan turn to finish",
        180_000,
        client.waitForFinish(agent.id, 170_000),
      );
      expect(finish.status).toBe("idle");

      const postSteerMessages = collector.messages.slice(messagesBeforeSteer);
      const resolved = postSteerMessages.find(
        (message) =>
          message.type === "agent_permission_resolved" &&
          message.payload.requestId === planRequestId,
      );
      expect(resolved, "the pending plan must be resolved by the message").toBeDefined();

      // The whole point: the turn continues rather than being restarted.
      expect(
        ...
        "answering a plan with a message must not cancel the turn",
      ).toHaveLength(0);

      // Proves the SDK actually read the steer that was queued behind the
      // blocked permission callback — the assumption the server fix rests on.
      // Joined, not matched per item: assistant text arrives as streaming
      // deltas, so a short marker is split across items ("P" + "LAN_REVIEWED").
      expect(
        getAssistantTexts(collector.messages, agent.id).join("").toUpperCase(),
        "Claude must act on the message that resolved the plan",
      ).toContain("PLAN_REVIEWED");

      collector.unsubscribe();
    } finally {
      await resources.client?.close().catch(() => undefined);
